package dev.dapps.ndastro.data.network.http
...
class NDAstroHttpClientBuilder @Inject constructor(
    ...
    fun build(): HttpClient {
        client = HttpClient(Android) {
            ...
            defaultRequest {
                url {
                    protocol = this@NDAstroHttpClientBuilder.protocol
                    host = this@NDAstroHttpClientBuilder.host
                }
    ...
    private suspend fun refreshToken(): String? {
        ...
        return try {
            ...
            if (response.status == HttpStatusCode.OK) {
                val tokenResponse = json.decodeFromString<LoginResponse>(response.bodyAsText())
                tokenManager.saveTokens(tokenResponse.accessToken?.token, tokenResponse.refreshToken?.token)
